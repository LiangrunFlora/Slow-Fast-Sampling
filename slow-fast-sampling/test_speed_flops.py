...
from cache import FeatureCache, FeatureCacheConfig 
# from hook import register_cache
import threading
...
from utils.generate_multi_phase_new import generate_multi_phase_new
...
def parse_args():
    ...
    parser.add_argument("--prompt_interval_steps", type=int, nargs='+', default=[1], 
                    help="List of steps intervals for caching prompts.")
    parser.add_argument("--gen_interval_steps", type=int, nargs='+', default=[1], 
    ...
    parser.add_argument("--avg_prompt_length", type=int, default=145, 
    ...
    parser.add_argument("--is_cache", action="store_true", help="Enable cache")
...
def run_experiment(gpu_id, args, prompt_interval_steps, gen_interval_steps, result_lock):
    ...
    if args.is_cache:
        ...
        #     test_flops=True
        # )
        print(f"Cache is enabled, prompt_interval_steps={prompt_interval_steps}, gen_interval_steps={gen_interval_steps}")
    else:
    ...
    result = (
        ...
        f"- FLOPs: {avg_flops_cached / 1e12:.4f} TFLOPS   MACs: {avg_macs_cached / 1e12:.4f} TMACs \n"
    )
    print(result)
    ...
    arg_run_time = sum(times) / len(times) / args.batch_size 
    print(f"Average generation time over {args.num_runs} runs: {arg_run_time:.3f} seconds")
    print(f"TPS: {args.gen_length/arg_run_time}")
    
    # Prepare data for CSV
    log_entry = {
        "arg_run_time_per_token": round(arg_run_time, 4),
        "avg_flops_per_token": round(avg_flops_cached / 1e12, 4),
        "avg_macs_per_token": round(avg_macs_cached / 1e12, 4),
        "prompt_interval_steps": prompt_interval_steps,
        "gen_interval_steps": gen_interval_steps,
        "transfer_ratio": args.transfer_ratio,
        "steps": args.steps,
