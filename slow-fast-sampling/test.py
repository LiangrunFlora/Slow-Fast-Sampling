...
def cache_hook_feature(
    ...
    if refresh_gen and refresh_prompt:
        # feature_cache.set_cache(layer_id=self.layer_id, feature_name="token", features=x[:, prompt_length:, :], cache_type="gen")
        # feature_cache.set_cache(layer_id=self.layer_id, feature_name="token", features=x[:, prompt_length:, :], cache_type="prompt")
        q, k, v = project(x)
        ...
        if transfer:
            ...
            k_gen = self.k_proj(x_gen_normed)
            index = refresh_index(k_gen,kv_cache_gen["k"],transfer_ratio,self.layer_id)
            ...
            v_gen_index = self.v_proj(x_gen_selected) 
            kv_cache_gen["k"] = k_gen
            kv_cache_gen["v"].scatter_(dim=1, index=index_expanded, src=v_gen_index)
